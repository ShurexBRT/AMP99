...
The native workflow also keeps the existing checks for three independent windows, docking, docked-group movement, Main minimize through the real WebView2 custom control, restore, Always on Top, `.wsz` handoff, association and MSI uninstall. The packaged smoke uses the WebView2 CDP endpoint only inside CI; it does not expose a production debug port. Spotify playback and OAuth remain owner/manual QA because CI must not contain personal credentials.

...
The native workflow also keeps the existing checks for three independent windows, docking, docked-group movement, Main minimize through the real custom control via Windows UI Automation, restore, Always on Top, `.wsz` handoff, association and MSI uninstall. Spotify playback and OAuth remain owner/manual QA because CI must not contain personal credentials.
